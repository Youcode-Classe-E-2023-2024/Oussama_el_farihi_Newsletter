<x-admin-layout>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-2">
                ...
                <div class="mt-6 p-2 bg-slate-100">
                    ...
                    <div class="flex space-x-2 mt-4 p-2">
                        ...
                        <form id="removeUserRoleForm-{{ $user_role->id }}"
                            class="px-4 py-2 bg-red-500 hover:bg-red-700 text-white rounded-md" method="POST"
                            action="{{ route('admin.users.roles.remove', [$user->id, $user_role->id]) }}">
                            @csrf
                            @method('DELETE')
                            <button type="button"
                                onclick="confirmRemoveUserRole('{{ $user_role->name }}', 'removeUserRoleForm-{{ $user_role->id }}')">{{
    ...
    <script>
        function confirmRemoveUserRole(roleName, formId) {
            Swal.fire({
                title: 'Are you sure?',
